...
export default function CommunicationLog({ 
  ...
  const loadSystemWechatSessions = React.useCallback(async () => {
    ...
    try {
      ...
      let sessionQuery = supabase.from('crm_wechat_session').select('*');
      if (customerId) {
        sessionQuery = sessionQuery.eq('customer_id', customerId);
      } else if (wechatIds.length > 0) {
        sessionQuery = sessionQuery.in('peer_wechat_id', wechatIds);
      }
      const { data: sessions } = await sessionQuery;
      let groupQuery = supabase.from('crm_wechat_group').select('*');
      if (customerId) {
    ...
    if (sourceSessions.length > 0) {
      return sourceSessions.map((session: any) => {
        const matchKeys = new Set([
          ...
          String(session.contact_name || ''),
          String(session.customer_name || '')
        ].filter(Boolean));
        ...
        return {
          ...
          name: session.contact_name || session.peer_wechat_id || session.customer_name || session.my_wechat_id || '未命名会话',
          messages,
          lastMessage: messages[messages.length - 1]?.content || session.last_msg || '',
          lastTime: messages[messages.length - 1]?.date || ''
        };
      });
    }
  ...
  const handleSaveSession = (data: any) => {
    ...
    const activeSessionSourceGroup = activeTab === 'wechat'
      ? (selectedWechatSession?.name || selectedWechatSession?.peer_wechat_id || '')
      : activeTab === 'wechat_group'
        ? (selectedGroupChat?.group_name || selectedGroupChat?.groupName || selectedGroupChat?.name || '')
        : '';
    onAddCommunication({
  ...
  const derivedGroupChats = useMemo(() => {
    ...
    if (sourceGroups.length > 0) {
      return sourceGroups.map((group: any) => {
        ...
        return {
          ...group,
          id: String(group.id || `wg_${group.group_id || group.group_name || Math.random()}`),
          name: group.group_name || group.name || '未命名群聊',
    ...
    try {
      if (isSupabaseConfigured() && customerId) {
        ...
        if (sessionSelectorMode === 'individual') {
          ...
          const matchedContact = (contacts || []).find((c) => String(c.wechatId || '') === peerWechatId);
          if (matchedContact?.id) payload.contact_id = matchedContact.id;
          const { error } = await supabase.from('crm_wechat_session').update(payload).eq('id', selectedId);
          if (error) throw error;
        } else {
          const { error } = await supabase.from('crm_wechat_group').update({ customer_id: customerId }).eq('id', selectedId);
          if (error) throw error;
        }
      }
