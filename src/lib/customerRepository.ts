...
export const resolveCustomerDbIdFromSupabase = async (customerId: string): Promise<number | null> => {
  ...
  const directId = Number.parseInt(id, 10);
  if (Number.isFinite(directId) && String(directId) === id) {
    return directId;
  }
  ...
  const { data, error } = await supabase
    ...
    .select('id')
    .in('customer_number', candidateNumbers)
    .limit(1)
    .maybeSingle();
...
export const fetchCustomerCommunicationsFromSupabase = async (customerId: string): Promise<CommunicationDetail[]> => {
  ...
  const dbCustomerId = await resolveCustomerDbIdFromSupabase(id);
...
export const saveCustomerCommunicationToSupabase = async (
  ...
  const dbCustomerId = await resolveCustomerDbIdFromSupabase(id);
  if (!dbCustomerId) throw new Error(`无法识别客户ID：${id}`);
  const supabase = getSupabaseClient();
  const now = new Date();
  const payload = {
    id: String(comm.id || `LOG${Date.now()}`),
  ...
  await updateCustomerLastContactInSupabase(String(dbCustomerId), '沟通记录更新');
...
export const saveCustomersSnapshotToSupabase = async (customers: Customer[]): Promise<Customer[]> => {
  ...
  const contactRows = customers.flatMap((customer) => {
    ...
    return (customer.contacts || []).map((contact) => ({
      id: contact.id || `CON${Date.now()}${Math.floor(Math.random() * 1000)}`,
      customer_id: dbCustomerId,
      name: contact.name || '',
  ...
  const followRows = customers.flatMap((customer) => {
    ...
    return (customer.followUps || []).map((follow) => ({
      id: follow.id || `FU${Date.now()}${Math.floor(Math.random() * 1000)}`,
