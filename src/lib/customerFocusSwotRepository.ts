...
export const saveCustomerFocusSwot = async (customerId: string, focusItems: DbFocusSwotData[]) => {
  ...
  if (focusIds.length > 0) {
    ...
    const { error: clearFocusError } = await supabase.from('crm_customer_focus_swot').delete().eq('customer_id', dbCustomerId);
  ...
  if (compIds.length > 0) {
    ...
    const { error: clearCompError } = await supabase.from('crm_customer_focus_competitor').delete().eq('customer_id', dbCustomerId);
