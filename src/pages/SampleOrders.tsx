...
export default function SampleOrders({ role, viewParams, navigateTo, goBack }: SampleOrdersProps) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const remote = await fetchSampleOrdersFromSupabase();
      setOrders(remote);
    } catch (error) {
  ...
  const handleSaveOrder = async (updatedData: SampleOrder, shouldClose = true) => {
    try {
      const payload = updatedData.id ? updatedData : { ...updatedData, id: `SAM${Date.now()}` };
  ...
  const filteredOrders = orders.filter(order => {
    ...
    return (
      (order.sampleNo || '').toLowerCase().includes(searchLower) ||
  ...
  const handleDeleteOrder = async (id: string) => {
    ...
    if (!order) return;
    const ok = await ensureDeleteAllowed({ record: order, entityName: '样品单' });
    if (!ok) return;
    try {
      await deleteSampleOrderFromSupabase(id);
      setOrders(orders.filter((item) => item.id !== id));
    } catch (error) {
      console.error('Error deleting sample order:', error);
      toast.error(`样品单删除失败：${(error as Error)?.message || '请检查 Supabase 配置'}`);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <h2 className="text-2xl font-bold text-gray-900">样品单管理</h2>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden flex flex-col h-[calc(100vh-12rem)]" onScroll={handleScroll}>
        {loading ? (
          ...
          <div className="h-full flex flex-col items-center justify-center text-gray-500">
            ...
            <p className="text-lg font-medium text-gray-900">暂无样品单数据</p>
            <p className="text-sm mt-1">点击右上角"新建样品单"创建</p>
          </div>
        ) : (
          <>
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredOrders.slice(0, displayCount).map(o => (
                <tr key={o.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4 font-bold text-indigo-600 cursor-pointer hover:underline" onClick={() => setSelectedOrder(o)}>{o.sampleNo}</td>
                  <td className="px-6 py-4 font-medium text-gray-900">{o.customerName}</td>
                  <td className="px-6 py-4 text-gray-600">{o.projectName || '-'}</td>
                  <td className="px-6 py-4 text-gray-600">{o.applicant}</td>
            ...
            <div key={o.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm space-y-3" onClick={() => setSelectedOrder(o)}>
              <div className="flex justify-between items-start">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-indigo-600 font-medium">{o.sampleNo}</p>
                </div>
                <span className={cn(
              ...
              <div className="grid grid-cols-2 gap-y-2 text-sm">
                ...
                <div>
                  ...
                  <p className="text-gray-900">{o.applicant}</p>
                </div>
                <div>
