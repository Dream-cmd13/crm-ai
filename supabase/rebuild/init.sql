...
alter table if exists public.crm_opportunity
  add column if not exists customer_type text;
alter table if exists public.crm_opportunity
...
create table if not exists crm_case_product_rel (
  ...
  product_id integer not null references ba_cpinfo(id) on delete cascade
);
create table if not exists crm_case_category_rel (
  id uuid primary key default gen_random_uuid(),
  case_id text not null references crm_case_library(id) on delete cascade,
  category_id integer not null references ba_cptype(id) on delete cascade
);
-- ========= COMPETITOR / SWOT =========
